-- BadgerIQ migration 020 — hourly auto-sync for API data-source connectors.
-- SECURITY DEFINER functions list connectors due for background sync (cross-tenant).

CREATE OR REPLACE FUNCTION connector_scheduled_sync()
RETURNS TABLE(connector_id uuid, tenant_id uuid)
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT c.connector_id, c.tenant_id
  FROM connectors c
  WHERE c.enabled = true
    AND COALESCE(c.kind, '') <> 'github-copilot-business'
    AND COALESCE((c.schedule_json->>'enabled')::boolean, true) = true
    AND NOT (
      c.status = 'syncing'
      AND c.last_sync_started_at IS NOT NULL
      AND c.last_sync_started_at > NOW() - interval '15 minutes'
    )
    AND (
      c.last_success_at IS NULL
      OR c.last_success_at < NOW() - (
        COALESCE((c.schedule_json->>'intervalMinutes')::int, 60) || ' minutes'
      )::interval
    );
$$;

GRANT EXECUTE ON FUNCTION connector_scheduled_sync() TO agentledger_api;

-- Copilot connections: hourly default (join connectors for in-flight sync guard).
CREATE OR REPLACE FUNCTION copilot_scheduled_connections()
RETURNS TABLE(connection_id uuid, tenant_id uuid)
LANGUAGE sql
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT c.connection_id, c.tenant_id
  FROM ai_provider_connections c
  JOIN connectors k ON k.connector_id = c.connector_id
  WHERE c.provider = 'github_copilot_business'
    AND k.enabled = true
    AND COALESCE((c.schedule_json->>'enabled')::boolean, true) = true
    AND NOT (
      k.status = 'syncing'
      AND k.last_sync_started_at IS NOT NULL
      AND k.last_sync_started_at > NOW() - interval '15 minutes'
    )
    AND (
      c.last_success_at IS NULL
      OR c.last_success_at < NOW() - (
        COALESCE((c.schedule_json->>'intervalMinutes')::int, 60) || ' minutes'
      )::interval
    );
$$;

GRANT EXECUTE ON FUNCTION copilot_scheduled_connections() TO agentledger_api;

-- Backfill empty schedules so existing connectors pick up hourly auto-sync.
UPDATE connectors
SET schedule_json = '{"intervalMinutes": 60, "enabled": true}'::jsonb
WHERE schedule_json = '{}'::jsonb OR schedule_json IS NULL;

UPDATE ai_provider_connections
SET schedule_json = '{"intervalMinutes": 60, "enabled": true}'::jsonb
WHERE schedule_json = '{}'::jsonb
   OR schedule_json = '{"frequency":"daily"}'::jsonb;
