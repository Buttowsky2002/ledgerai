...
| `risk-enrichment` | `BADGERIQ_RISK_ENRICH_ENABLED`, `_LOOKBACK_HOURS`, `_MIN_CALLS`, `_MIN_CONFIDENCE`, `_INTERVAL_SEC`; self-hosted LLM (OpenAI-compatible, no external AI API — ADR-050): `BADGERIQ_LLM_BASE_URL`, `BADGERIQ_LLM_MODEL`, `BADGERIQ_LLM_TIMEOUT_S`, `BADGERIQ_AI_MAX_TOKENS`, 🔒 `BADGERIQ_LLM_API_KEY` (optional gateway bearer) |
