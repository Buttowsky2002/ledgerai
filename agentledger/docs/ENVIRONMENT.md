...
| `BADGERIQ_ANALYTICS_BACKEND` | `clickhouse` | Analytics store: `clickhouse` (full stack) or `postgres` (Cloud Run MVP / pilot without ClickHouse; requires migration `023_analytics_mvp.sql`). Pilot ECS currently sets `postgres`. |
