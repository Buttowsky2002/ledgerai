...
The obvious approach — add `toolchain go1.26.6` to each go.mod — **breaks
...
Instead, `GOTOOLCHAIN=go1.26.6` is set **on the govulncheck step only**:
...
- The Go toolchain pin for govulncheck (`go1.26.6`) is a value to bump as new
