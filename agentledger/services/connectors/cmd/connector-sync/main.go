...
func main() {
	...
	syncEvery := time.Duration(envInt("BADGERIQ_SYNC_INTERVAL_SEC", 3600)) * time.Second
	...
	srv := &http.Server{Addr: env("BADGERIQ_CONNECTOR_ADDR", ":8092"), Handler: mux, ReadHeaderTimeout: 10 * time.Second}
	go func() {
