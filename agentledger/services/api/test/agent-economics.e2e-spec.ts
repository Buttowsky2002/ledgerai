...
describe('Agent economics (/v1/analytics/agent-economics)', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.withTenant(tenantA, (tx) =>
      tx.tenant.create({ data: { tenantId: tenantA, name: 'econ-a' } }),
    );
    await prisma.withTenant(tenantB, (tx) =>
      tx.tenant.create({ data: { tenantId: tenantB, name: 'econ-b' } }),
    );
  ...
  it('rejects unauthenticated (401)', async () => {
    expect((await request(app.getHttpServer()).get('/v1/analytics/agent-economics')).status).toBe(
      401,
    );
  });
