...
describe('Analytics (ClickHouse MVs)', () => {
  ...
  it('viewer sees their tenant spend (sum matches seeded cost)', async () => {
    ...
    const total = res.body.reduce(
      (s: number, r: { cost_usd: number }) => s + Number(r.cost_usd),
      0,
    );
  ...
  it('unit-economics excludes low-confidence outcomes from headline numbers', async () => {
    ...
    await insertCH('agent_runs', [
      ...
      },
      {
        ...
        app_id: 'app',
        user_id: 'u',
        started_at: '2026-06-10 09:10:00',
        ended_at: '2026-06-10 09:15:00',
        status: 'completed',
        total_cost_usd: 8.0,
        total_tokens: 10,
        llm_calls: 1,
    ...
    await insertCH('outcomes', [
      ...
      },
      {
        ...
        user_id: 'u',
        run_id: 'ue-r2',
        business_value_usd: 300,
        quality_score: 0.4,
        attribution_confidence: 0.3,
        completion_status: 'solved',
      },
    ]);
    const sum = (body: { [k: string]: unknown }[], key: string) =>
      body.reduce((s, r) => s + Number(r[key]), 0);

    // Unfiltered: both outcomes, ai_cost = 2 + 8, cost_per_outcome = 10/2 = 5.
    const all = await request(app.getHttpServer())
      .get('/v1/analytics/unit-economics')
      .query(WIDE)
      .set(bearer(await tok(tenantUE, 'viewer')));
