...
describe('Bulk import /v1/import/events', () => {
  ...
  beforeAll(async () => {
    ...
    await prisma.withTenant(tenantA, (tx) =>
      tx.tenant.create({ data: { tenantId: tenantA, name: 'import-a' } }),
    );
    await prisma.withTenant(tenantB, (tx) =>
      tx.tenant.create({ data: { tenantId: tenantB, name: 'import-b' } }),
    );
  });
  ...
  const tok = (tenant: string, role = 'admin') =>
  ...
  const post = async (tenant: string, body: object, role = 'admin') =>
    request(app.getHttpServer())
      ...
      .set(bearer(await tok(tenant, role)))
      .send(body);

  it('rejects unauthenticated (401)', async () => {
    expect(
      (await request(app.getHttpServer()).post('/v1/import/events').send({ events: [] })).status,
    ).toBe(401);
  });
  ...
  it('imports usage + outcome + risk rows and lands them in ClickHouse', async () => {
    const res = await post(tenantA, {
      events: [
        ...
        {
          idempotency_key: k('o1'),
          outcome_type: 'merged_pr',
          outcome_value_usd: 500,
          run_id: 'run-1',
        },
        {
    ...
    expect(
      await chCount(
        `SELECT count() c FROM agentledger.llm_calls WHERE tenant_id = '${tenantA}' AND call_id = 'imp_${k('u1')}'`,
      ),
    ).toBe(1);
    expect(
      await chCount(
        `SELECT count() c FROM agentledger.outcomes WHERE tenant_id = '${tenantA}' AND outcome_id = 'imp_${k('o1')}_out'`,
      ),
    ).toBe(1);
    expect(
      await chCount(
        `SELECT count() c FROM agentledger.risk_events WHERE tenant_id = '${tenantA}' AND event_id = 'imp_${k('r1')}_risk'`,
      ),
    ).toBe(1);
  });
  ...
  it('is idempotent: re-importing the same batch skips everything (no double counting)', async () => {
    const batch = {
      events: [{ idempotency_key: k('u1'), model: 'gpt-4o', input_tokens: 100, cost_usd: 1.5 }],
    };
  ...
  it('dry run reports the plan without writing', async () => {
    ...
    expect(
      await chCount(
        `SELECT count() c FROM agentledger.llm_calls WHERE tenant_id = '${tenantA}' AND call_id = 'imp_${k('dry1')}'`,
      ),
    ).toBe(0);
  });
  ...
  it('isolates tenants: tenant B importing its own key does not appear under tenant A', async () => {
    const res = await post(tenantB, {
      events: [{ idempotency_key: k('b1'), model: 'gpt-4o', input_tokens: 1 }],
    });
    expect(res.status).toBe(201);
    ...
    expect(
      await chCount(
        `SELECT count() c FROM agentledger.llm_calls WHERE tenant_id = '${tenantB}' AND call_id = 'imp_${k('b1')}'`,
      ),
    ).toBe(1);
    // ...and never for A.
    expect(
      await chCount(
        `SELECT count() c FROM agentledger.llm_calls WHERE tenant_id = '${tenantA}' AND call_id = 'imp_${k('b1')}'`,
      ),
    ).toBe(0);
  });
});
