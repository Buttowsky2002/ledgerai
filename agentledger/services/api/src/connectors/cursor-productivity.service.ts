...
export class CursorProductivityService {
  ...
  async getProductivitySummary(
    ...
    if (rows.length === 0 && !commitAttribution) {
      return null;
    }
    ...
    for (const row of rows) {
      ...
      if (roi.estimatedValueUsd <= 0 && roi.linesAccepted <= 0) {
        continue;
      }
      activeUserDays += 1;
    ...
    if (activeUserDays === 0 && !commitAttribution) {
      return null;
    }
  ...
  toOutcomeBreakdownRow(
    summary: CursorProductivitySummary,
    cursorSpendUsd: number,
  ): {
    outcomeType: string;
    outcomes: number;
