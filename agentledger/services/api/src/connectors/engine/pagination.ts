...
export function extractPage(response: unknown, config: PaginationConfig | undefined): PageResult {
  ...
  switch (config.type) {
    case 'cursor': {
      const nextCursor = config.cursorPath
        ? String(getPath(response, config.cursorPath) ?? '')
        : undefined;
      return { items, nextCursor: nextCursor || undefined, hasMore: !!nextCursor };
    }
    case 'next_url': {
      const nextUrl = config.nextUrlPath
        ? String(getPath(response, config.nextUrlPath) ?? '')
        : undefined;
      return { items, nextUrl: nextUrl || undefined, hasMore: !!nextUrl };
    }
    case 'response_token': {
      const nextToken = config.tokenPath
        ? String(getPath(response, config.tokenPath) ?? '')
        : undefined;
...
export function buildPaginationParams(
  ...
  switch (config.type) {
    ...
    case 'page':
      if (config.pageParam) {
        params[config.pageParam] = String(state.page ?? 1);
      }
      break;
    case 'offset':
      if (config.offsetParam) {
        params[config.offsetParam] = String(state.offset ?? 0);
      }
      break;
    case 'response_token':
      if (state.token && config.tokenParam) {
        params[config.tokenParam] = state.token;
      }
      break;
  }
  if (config.limitParam) {
    params[config.limitParam] = String(pageSize);
  }
  return params;
}
