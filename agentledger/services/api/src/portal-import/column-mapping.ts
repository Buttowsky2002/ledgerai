...
function applyRoles(
  headers: string[],
  roles: Partial<Record<string, PortalColumnRole>>,
): MappingSuggestion {
  const byHeader: Record<string, PortalColumnRole | undefined> = {};
  for (const h of headers) {
    byHeader[h] = roles[h] ?? 'skip';
  }
...
export function suggestColumnMapping(
  ...
  if (format === 'anthropic_console' || format === 'unknown') {
    ...
    if (!suggestion.mapping.date) {
      suggestion.missingRequired.push('date');
    }
    return suggestion;
  }
...
export function resolveColumnMapping(
  ...
  if (cost < 0) {
    return { resolved: null, error: `cost column "${mapping.cost}" not found in CSV headers` };
  }
...
export function findHeaderRowIndex(grid: string[][]): number {
  ...
  for (let i = 0; i < scan; i++) {
    ...
    if (format === 'anthropic_spend_report') {
      score = 100;
    } else if (format === 'anthropic_console') {
      score = 90;
    } else if (format === 'cursor_analytics' || format === 'claude_code_lines') {
      score = 10;
    } else {
