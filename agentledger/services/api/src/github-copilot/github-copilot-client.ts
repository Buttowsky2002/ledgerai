...
export class GitHubCopilotClient {
  ...
  async fetchMembers(): Promise<{ id: number; login: string }[]> {
    ...
    for (;;) {
      ...
      for (const m of batch) {
        if (m.id && m.login) {
          members.push({ id: m.id, login: m.login });
        }
      }
      if (batch.length < 100) {
        break;
  ...
  async fetchTeams(): Promise<{ slug: string; name: string }[]> {
    ...
    for (;;) {
      ...
      for (const t of batch) {
        if (t.slug) {
          teams.push({ slug: t.slug, name: t.name ?? t.slug });
        }
      }
      if (batch.length < 100) {
        break;
  ...
  async fetchMembersDetailed(): Promise<CopilotMemberRow[]> {
    ...
    for (;;) {
      ...
      >('GET', `/orgs/${encodeURIComponent(this.orgSlug)}/members?per_page=100&page=${page}`);
      if (!Array.isArray(batch) || batch.length === 0) {
        break;
      }
      for (const m of batch) {
  ...
  private async fetchMetricsReport(path: string): Promise<CopilotUsageRow[]> {
    ...
    const defaultDay = str(meta.report_end_day ?? meta.report_day ?? meta.report_start_day).slice(
      0,
      10,
    );
    ...
    if (Array.isArray(meta)) {
      return this.normalizeUsageRows(meta, path, defaultDay);
    }
    if (Array.isArray(meta.data)) {
      return this.normalizeUsageRows(meta.data as unknown[], path, defaultDay);
    }
    return [];
  }
    ...
    if (text.trimStart().startsWith('[') || text.trimStart().startsWith('{')) {
      try {
        ...
        if (Array.isArray(parsed)) {
          return this.normalizeUsageRows(parsed, sourcePath, defaultDay);
        }
    ...
    for (const line of text.split(/\r?\n/)) {
      ...
      try {
        ...
        if (obj && typeof obj === 'object') {
          ...
          if (defaultDay && !rec.day && !rec.date) {
            rec.day = defaultDay;
          }
  ...
  private normalizeUsageRecord(r: Record<string, unknown>, sourcePath: string): CopilotUsageRow {
    ...
    return {
      ...
      suggestionsCount: num(
        r.code_generation_activity_count ??
          r.suggestions_count ??
          r.total_suggestions ??
          r.suggestions,
      ),
      acceptancesCount: num(
        r.code_acceptance_activity_count ??
          r.acceptances_count ??
          r.total_acceptances ??
          r.acceptances,
      ),
      ...
      engagedUsers: num(r.weekly_active_users ?? r.engaged_users ?? r.total_engaged_users),
      chatTurns: num(
