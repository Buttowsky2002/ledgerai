...
Next.js 14 (App Router) FinOps dashboard for AgentLedger. Pages are React Server Components
...
- **Dev**: set `AGENTLEDGER_DEV_TENANT_ID` (and run the API with
  `AGENTLEDGER_DEV_TRUST_HEADER=true`); the server client sends `x-tenant-id` so the dashboard
...
AGENTLEDGER_API_URL=http://localhost:8094 AGENTLEDGER_DEV_TENANT_ID=<tenant-uuid> npm run dev
# → http://localhost:3000
```
...
|----------|---------|---------|
| `AGENTLEDGER_API_URL` | `http://localhost:8094` | Control-plane API base URL (server-side). |
| `AGENTLEDGER_DEV_TENANT_ID` | _(unset)_ | Dev only: tenant sent via `x-tenant-id`. |
