...
Next.js 14 (App Router) FinOps dashboard for LedgerAI. Pages are React Server Components
...
- **Dev**: set `LEDGERAI_DEV_TENANT_ID` (and run the API with
  `LEDGERAI_DEV_TRUST_HEADER=true`); the server client sends `x-tenant-id` so the dashboard
...
LEDGERAI_API_URL=http://localhost:8094 LEDGERAI_DEV_TENANT_ID=<tenant-uuid> npm run dev
# → http://localhost:3000
```

## Environment

Variables use the `LEDGERAI_*` prefix. The legacy `AGENTLEDGER_*` names are still
read as a fallback (deprecated aliases) so existing setups keep working.

| Variable | Default | Purpose |
|----------|---------|---------|
| `LEDGERAI_API_URL` | `http://localhost:8094` | Control-plane API base URL (server-side). |
| `LEDGERAI_DEV_TENANT_ID` | _(unset)_ | Dev only: tenant sent via `x-tenant-id`. |
